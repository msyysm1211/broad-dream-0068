...
export default function Home() {
  const envVars = {
    ESA_CACHE_GW_GATEWAY_ENDPOINT:
      process.env.ESA_CACHE_GW_GATEWAY_ENDPOINT ?? "未设置",
    ...
    <main style={{ padding: "2rem" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            <tr style={{ borderBottom: "1px solid #eee" }}>
              ...
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                {envVars.ESA_CACHE_GW_GATEWAY_ENDPOINT}
              </td>
            </tr>
            <tr style={{ borderBottom: "1px solid #eee" }}>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                ESA_CACHE_GW_AUTH_KEY
              </td>
              <td style={{ padding: "0.5rem" }}>
                已派生的 authKey（HMAC-SHA256）
              </td>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                {envVars.ESA_CACHE_GW_AUTH_KEY}
              </td>
            </tr>
            <tr style={{ borderBottom: "1px solid #eee" }}>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
              ...
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                {envVars.ESA_CACHE_GW_ALIUID}
              </td>
            </tr>
            <tr style={{ borderBottom: "1px solid #eee" }}>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                ESA_CACHE_GW_ROUTINENAME
              </td>
              <td style={{ padding: "0.5rem" }}>Routine 名称</td>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                {envVars.ESA_CACHE_GW_ROUTINENAME}
              </td>
            </tr>
            <tr style={{ borderBottom: "1px solid #eee" }}>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                ESA_CACHE_GW_VERSION
              </td>
              <td style={{ padding: "0.5rem" }}>版本号</td>
              <td style={{ padding: "0.5rem", fontFamily: "monospace" }}>
                {envVars.ESA_CACHE_GW_VERSION}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      >
        ...
        <p style={{ fontSize: "0.85rem", color: "#666" }}>
          公式:{" "}
          <code>md5(authKey + expires + aliuid + routinename + version)</code>
        </p>
        ...
        <p style={{ margin: "0.5rem 0" }}>
          ...
          >{`${authKey}${expires}${aliuid}${routinename}${version}`}</code>
        </p>
        <p style={{ margin: "0.5rem 0" }}>
